...
top_dir=/Users/chaiy3/Data/Audiovisual_motion # replace with your own data directory
cd $top_dir

for patDir in 190530OWU_SAM.goodlars; do
{
	cd ${top_dir}/${patDir}
	for runDir in anat lars; do # 
	{	
		if [ -d ${top_dir}/${patDir}/${runDir} ]; then
			...
			{	
				...
				3dTstat -overwrite -mean -prefix mean.${subj}.nii all_runs.$subj.nii # note TRs that were not censored
