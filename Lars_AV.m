...
taskTypes = [1 2 3]; % 1:audio motion; 2:visual motion; 3:congruent audiovisual motion; 4:incongruent audiovisual motion
...
Screen('DrawLines', window, allCoords, lineWidthPix, white, [xCenter yCenter+fixYPos], 2);
...
while elapsedTime < restSecs
    ...
    [ pressed, firstPress]=KbQueueCheck();
    % If the user has pressed a key, then display its code number and name.
    if pressed && firstPress(escapeKey)
        KbQueueStop();
        KbQueueRelease();
...
% display response result to subject
textString = ['Thank you! \n You detected ' num2str(round(expLog.correctRate*100)) '% movements correctly.'];
